...
**AC-3 plays from every container PsyMP3 demuxes; E-AC-3 is recognised but not yet decoded.** Written from [ATSC A/52:2012](docs/), which is publicly available, so unlike MLP this one is ISC and in-tree. `ffmpeg` is used strictly as an oracle: it generates streams to decode and its own PCM output is what a decode is diffed against. Its source is GPL-3.0-only and is never read; neither is liba52's.
...
- **Still to do:** downmixing and dynamic range control, then Annex E: the `audfrm` frame layer, the audio block's changed syntax, AHT, spectral extension, enhanced coupling, transient pre-noise processing and dependent substreams. ffmpeg's encoder emits only the core Annex E path at every bitrate and layout — no AHT, spectral extension, enhanced coupling or transient pre-noise processing — so those tools can be verified only against the spec's own arithmetic.
...
- **CodecID maps to the names `CodecRegistry` dispatches on** (`codecNameForId`), and a codec PsyMP3 has no decoder for maps to nothing *on purpose*. E-AC-3 and DTS are the notable absences and they are common in `.mkv` (AC-3, commoner still, is decoded in tree); returning empty lets a file be refused by name rather than opened and failed at its first packet. `A_AAC` matches with or without a profile suffix, since one decoder covers the family and the profile comes back out of the AudioSpecificConfig anyway.
...
Two known gaps remain: `A_MS/ACM` (a `WAVEFORMATEX` in `CodecPrivate`) is unmapped, and E-AC-3 and DTS have no decoder in tree, so a `.mkv` whose only audio is one of those is skipped.
