...
Still to come: block and lacing parsing (four lacing modes, of which EBML lacing uses signed varint deltas), seeking through `Cues` with a scan fallback for the files that ship without them, `Tags`, and the `registerDemuxer`/`MediaFactory` wiring. Two known gaps: a file that writes `Tracks` *after* its clusters needs `SeekHead` to be followed, which is not done yet, and `A_MS/ACM` (a `WAVEFORMATEX` in `CodecPrivate`) is unmapped.
