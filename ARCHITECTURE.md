...
**In progress — the bitstream parses, nothing is wired to playback yet.** Written from [ATSC A/52:2012](docs/), which is publicly available, so unlike MLP this one is ISC and in-tree. `ffmpeg` is used strictly as an oracle: it generates streams to decode and its own PCM output is what a decode is diffed against. Its source is GPL-3.0-only and is never read; neither is liba52's.
...
- **Still to do:** the IMDCT with its 512/256-point block switching, windowing and overlap-add, downmixing, and then Annex E in full — AHT, spectral extension, enhanced coupling and dependent substreams.
...
- **CodecID maps to the names `CodecRegistry` dispatches on** (`codecNameForId`), and a codec PsyMP3 has no decoder for maps to nothing *on purpose*. AC-3, E-AC-3 and DTS are the notable absences and they are common in `.mkv`; returning empty lets a file be refused by name rather than opened and failed at its first packet. `A_AAC` matches with or without a profile suffix, since one decoder covers the family and the profile comes back out of the AudioSpecificConfig anyway.
...
Two known gaps remain: `A_MS/ACM` (a `WAVEFORMATEX` in `CodecPrivate`) is unmapped, and AC-3, E-AC-3 and DTS have no decoder in tree, so a `.mkv` whose only audio is one of those is skipped.
