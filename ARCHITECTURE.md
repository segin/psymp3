...
- `src/codecs/`: Decode AAC, xHE-AAC, FLAC, MP3, Opus, Vorbis, PCM-family, G.711, and G.722 streams into PCM. xHE-AAC (MPEG-D USAC, audio object type 42) is a separate decoder rather than a mode of the AAC one: libfaad2 stops at object type 27 and cannot decode USAC at all, so `XHEAACCodec` drives FDK-AAC instead. Which of the two a stream gets is decided in `BoxParser` by reading the audioObjectType out of the AudioSpecificConfig, since both share esds objectTypeIndication 0x40. FDK's headers define ID_SCE/ID_CPE/ID_LFE, which collide with faad2's, so the FDK calls live behind a plain-C shim (`codecs/aac/xhe_fdk.cpp`) built as its own translation unit — the same treatment stb_vorbis gets, and for the same reason. Vorbis uses the vendored stb_vorbis (`third_party/stb`, compiled as its own C object) in pushdata mode: the Ogg demuxer delivers de-paged packets, and `VorbisCodec` wraps each one back into minimal Ogg page framing (real lacing/continuation and page CRC — stb validates CRCs during its post-seek resync) before feeding the decoder. No libvorbis at link time; libogg remains for container parsing.
