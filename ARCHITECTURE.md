...
**In progress.** `.mka`, `.mkv` and `.webm` do not play yet: the pieces below exist and are tested, but nothing registers a demuxer, so `MediaFactory` still does not recognise the extensions. What is here is the bottom two layers.
...
Still to come: `Tags`, and the `registerDemuxer`/`MediaFactory` wiring. Two known gaps: a file that writes `Tracks` *after* its clusters needs `SeekHead` to be followed, which is not done yet, and `A_MS/ACM` (a `WAVEFORMATEX` in `CodecPrivate`) is unmapped.
