...
bool OggDemuxer::parseContainer() {
    ...
    while (in_headers) {
        ...
        Debug::log("ogg", "OggDemuxer::parseContainer() getNextPage returned: ", result, " (1=success, 0=EOF/error)");
        if (result != 1) {
            Debug::log("ogg", "OggDemuxer::parseContainer() failed after ", page_count, " pages, streams found: ", m_streams.size());
            return !m_streams.empty(); // OK if we found at least one stream
        }
        page_count++;
        
        int serial = ogg_page_serialno(&page);
        Debug::log("ogg", "OggDemuxer::parseContainer() page ", page_count, " serial=", serial, " bos=", ogg_page_bos(&page));
        
        // Is this a BOS page?
        if (ogg_page_bos(&page)) {
            Debug::log("ogg", "OggDemuxer::parseContainer() BOS page detected, creating stream manager");
            ...
            Debug::log("ogg", "OggDemuxer::parseContainer() getPacket returned ", got_packet, " bytes=", got_packet ? packet.bytes : 0);
            if (got_packet) {
                auto parser = CodecHeaderParser::create(&packet);
                Debug::log("ogg", "OggDemuxer::parseContainer() CodecHeaderParser::create returned ", (parser != nullptr ? "valid parser" : "nullptr"));
                if (parser) {
                    ...
                    if (m_primary_serial < 0) {
                        ...
                        Debug::log("ogg", "OggDemuxer::parseContainer() set primary_serial=", serial);
                    }
                }
            }
            
            m_streams[serial] = std::move(stream);
            Debug::log("ogg", "OggDemuxer::parseContainer() streams count=", m_streams.size(), " parsers count=", m_parsers.size());
        } else {
            // Not a BOS page - submit to appropriate stream
            Debug::log("ogg", "OggDemuxer::parseContainer() non-BOS page for serial=", serial);
            auto it = m_streams.find(serial);
            if (it != m_streams.end()) {
                it->second->submitPage(&page);
                Debug::log("ogg", "OggDemuxer::parseContainer() submitted page to stream manager");
                
                // Check if headers complete
                auto pit = m_parsers.find(serial);
                bool headers_complete_before = pit != m_parsers.end() ? pit->second->isHeadersComplete() : true;
                Debug::log("ogg", "OggDemuxer::parseContainer() headers_complete_before=", headers_complete_before);
                
                if (pit != m_parsers.end() && !pit->second->isHeadersComplete()) {
                    ogg_packet packet;
                    int packet_count = 0;
                    while (it->second->getPacket(&packet)) {
                        Debug::log("ogg", "OggDemuxer::parseContainer() got packet ", packet_count, " bytes=", packet.bytes);
                        pit->second->parseHeader(&packet);
                        packet_count++;
                        if (pit->second->isHeadersComplete()) {
                            Debug::log("ogg", "OggDemuxer::parseContainer() headers now complete after packet ", packet_count);
                            break;
                        }
                    }
                    if (packet_count == 0) {
                        Debug::log("ogg", "OggDemuxer::parseContainer() no packets available from stream manager");
                    }
                }
            } else {
                Debug::log("ogg", "OggDemuxer::parseContainer() no stream found for serial=", serial);
            }
            ...
            for (auto& p : m_parsers) {
                ...
                Debug::log("ogg", "OggDemuxer::parseContainer() parser for serial=", p.first, " isHeadersComplete=", complete);
                if (!complete) {
                    in_headers = true;
                    break;
                }
            }
            Debug::log("ogg", "OggDemuxer::parseContainer() in_headers=", in_headers, " (if false, will exit loop)");
        }
    }
