/*
 * FrameParser.cpp - FLAC frame parsing implementation
 ...
 * The following bit patterns are explicitly forbidden by RFC 9639 and must be
 * rejected:
 *
...
FrameParser::FrameParser(BitstreamReader *reader, CRCValidator *crc)
    : m_reader(reader), m_crc(crc), m_last_sync_position(0),
      m_computing_header_crc(false), m_computed_header_crc(0) {}

FrameParser::~FrameParser() {}

/**
 * Find frame sync pattern (0xFFF8-0xFFFF)
 * Per RFC 9639 Section 9: Frame sync is 14 bits of 1 followed by 2 reserved
 * bits Valid sync patterns: 0xFFF8, 0xFFF9, 0xFFFA, 0xFFFB, 0xFFFC, 0xFFFD,
 * 0xFFFE, 0xFFFF Sync must be on byte boundary
 *
 * NOTE: The demuxer already provides chunks starting at the sync code.
 * This function verifies the sync is present and positions the reader
 * correctly.
 */
...
bool FrameParser::parseFrameHeader(FrameHeader &header) {
  ...
  Debug::log("flac_codec",
             "[parseFrameHeader] blocking_strategy=", blocking_strategy,
  ...
  if (!checkForbiddenSampleRateBits(sample_rate_bits)) {
    Debug::log("flac_codec", "Forbidden sample rate bits detected: 0b%04b",
               sample_rate_bits);
    return false;
  }
  ...
  if (block_size_bits == 0b0110) {
    ...
    if (header.block_size == 0 && block_size_bits != 0b0110 &&
        block_size_bits != 0b0111) {
      Debug::log("flac_codec", "Reserved block size bits: 0x", std::hex,
                 block_size_bits, std::dec);
      return false;
    }
  }
  ...
  if (header.bit_depth == 0 && bit_depth_bits != 0) {
    Debug::log("flac_codec", "Reserved bit depth bits: 0x", std::hex,
               bit_depth_bits, std::dec);
    return false;
  }
  ...
  if (m_computed_header_crc != header.crc8) {
    Debug::log("flac_codec",
               "Frame header CRC-8 mismatch: computed=0x%02X, expected=0x%02X",
  ...
  Debug::log("flac_codec",
             "Parsed frame header: block_size=%u, sample_rate=%u, channels=%u, "
             "bit_depth=%u",
             header.block_size, header.sample_rate, header.channels,
             header.bit_depth);
 ...
 * 7 bytes: 11111110 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx (36
 * bits)
 */
bool FrameParser::parseCodedNumber(FrameHeader &header) {
  ...
  if ((first_byte & 0x80) == 0) {
    ...
    Debug::log("flac_codec", "Invalid UTF-8 coded number first byte: 0x%02X",
               first_byte);
    return false;
  }
  ...
  for (uint32_t i = 1; i < num_bytes; i++) {
    ...
    if (!m_reader->readBits(cont_byte, 8)) {
      Debug::log("flac_codec",
                 "Failed to read coded number continuation byte %u", i);
      return false;
    }
    ...
    if ((cont_byte & 0xC0) != 0x80) {
      Debug::log("flac_codec", "Invalid UTF-8 continuation byte: 0x%02X",
                 cont_byte);
      return false;
    }
...
bool FrameParser::parseUncommonBlockSize(FrameHeader &header, uint32_t bits) {
  ...
  if (!m_reader->readBits(size_minus_one, bits)) {
    Debug::log("flac_codec", "Failed to read uncommon block size (%u bits)",
               bits);
    return false;
  }
  ...
  if (!checkForbiddenBlockSize(header.block_size)) {
    Debug::log("flac_codec", "Forbidden block size detected: %u",
               header.block_size);
    return false;
  }
...
bool FrameParser::parseUncommonSampleRate(FrameHeader &header, uint32_t bits) {
  ...
  if (!m_reader->readBits(rate, bits)) {
    Debug::log("flac_codec", "Failed to read uncommon sample rate (%u bits)",
               bits);
    return false;
  }
  ...
  Debug::log("flac_codec", "Parsed uncommon sample rate: %u Hz",
             header.sample_rate);
  return true;
}
...
/**
 * Parse frame footer
 * Per RFC 9639 Section 9: After all subframes, align to byte boundary with zero
 * padding, then read 16-bit CRC-16
 */
bool FrameParser::parseFrameFooter(FrameFooter &footer) {
  ...
  if (!m_reader->isAligned()) {
    ...
    if (bit_position != 0) {
      ...
      if (padding != 0) {
        Debug::log("flac_codec", "Warning: Non-zero padding bits: 0x%X",
                   padding);
        // Per RFC 9639, non-zero padding may indicate corruption but we
        // continue
      }
    }
  }
 ...
 */
bool FrameParser::validateFrame(const FrameHeader &header,
                                const FrameFooter &footer) {
  // Get computed CRC-16
...
bool FrameParser::validateSampleRate(uint32_t sample_rate) const {
  ...
  if (sample_rate > 1048575) {
    Debug::log("flac_codec", "Invalid sample rate: %u (max 1048575 Hz)",
               sample_rate);
    return false;
  }
  return true;
 ...
 */
bool FrameParser::checkForbiddenSampleRateBits(
    uint32_t sample_rate_bits) const {
