...
ResidualDecoder::ResidualDecoder(BitstreamReader *reader)
    : m_reader(reader), m_last_error(nullptr) {}

bool ResidualDecoder::decodeResidual(int32_t *output, uint32_t block_size,
  ...
  // RFC 9639 Section 9.2: block_size must be evenly divisible by
  // 2^partition_order
  ...
  if (block_size % partition_count != 0) {
    ...
    Debug::log("residual_decoder", "Block size ", block_size,
               " not divisible by ", partition_count);
    return false;
  }

  // Samples per partition is block_size / partition_count
  // Per RFC 9639: first partition has (block_size / partition_count) -
  // predictor_order samples Other partitions have (block_size /
  ...
  if (output_offset != residual_count) {
    ...
    Debug::log("residual_decoder",
               "Residual count mismatch: got %u, expected %u", output_offset,
               residual_count);
    return false;
  }
...
bool ResidualDecoder::parseResidualHeader(CodingMethod &method,
                                          uint32_t &partition_order) {
...
bool ResidualDecoder::decodePartition(int32_t *output,
  ...
  for (uint32_t i = 0; i < info.sample_count; ++i) {
    ...
    if (!decodeRiceCode(value, info.rice_parameter)) {
      Debug::log("residual_decoder", "Rice decode failed at sample ", i, " of ",
                 info.sample_count);
      return false;
    }
...
bool ResidualDecoder::decodeEscapedPartition(int32_t *output,
                                             const PartitionInfo &info) {
