/*
 * BitstreamReader.cpp - FLAC bitstream reading implementation
...
uint64_t BitstreamReader::getBitPosition() const { return m_total_bits_read; }
...
bool BitstreamReader::readUTF8_5byte(uint64_t &value) {
  ...
  if (!readBits(byte1, 8) || !readBits(byte2, 8) || !readBits(byte3, 8) ||
      !readBits(byte4, 8) || !readBits(byte5, 8)) {
    return false;
  }
...
bool BitstreamReader::readUTF8_6byte(uint64_t &value) {
  ...
  if (!readBits(byte1, 8) || !readBits(byte2, 8) || !readBits(byte3, 8) ||
      !readBits(byte4, 8) || !readBits(byte5, 8) || !readBits(byte6, 8)) {
    return false;
  }
