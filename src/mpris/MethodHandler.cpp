...
DBusHandlerResult MethodHandler::handleMessage(DBusConnection* connection, DBusMessage* message) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
DBusHandlerResult MethodHandler::handleMessage_unlocked(DBusConnection* connection, DBusMessage* message) {
    ...
    if (!interface_name || !member_name) {
        logError_unlocked("handleMessage", "Missing interface or member name in D-Bus message");
        return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
    }
    ...
    std::string method_key = std::string(interface_name) + "." + std::string(member_name);
    ...
    if (handler_it != m_method_handlers.end()) {
        try {
            ...
            logError_unlocked("handleMessage", "Exception in method handler: " + std::string(e.what()));
...
void MethodHandler::initializeMethodHandlers_unlocked() {
    ...
    m_method_handlers[std::string(MPRIS_MEDIAPLAYER2_INTERFACE) + ".Raise"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleRaise_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_MEDIAPLAYER2_INTERFACE) + ".Quit"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleQuit_unlocked(conn, msg); };
    
    // MPRIS MediaPlayer2.Player interface methods
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".Play"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handlePlay_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".Pause"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handlePause_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".Stop"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleStop_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".PlayPause"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handlePlayPause_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".Next"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleNext_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".Previous"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handlePrevious_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".Seek"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleSeek_unlocked(conn, msg); };
    
    m_method_handlers[std::string(MPRIS_PLAYER_INTERFACE) + ".SetPosition"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleSetPosition_unlocked(conn, msg); };
    
    // D-Bus Properties interface methods
    m_method_handlers[std::string(DBUS_PROPERTIES_INTERFACE) + ".Get"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleGetProperty_unlocked(conn, msg); };
    
    m_method_handlers[std::string(DBUS_PROPERTIES_INTERFACE) + ".Set"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleSetProperty_unlocked(conn, msg); };
    
    m_method_handlers[std::string(DBUS_PROPERTIES_INTERFACE) + ".GetAll"] = 
        [this](DBusConnection* conn, DBusMessage* msg) { return handleGetAllProperties_unlocked(conn, msg); };
}

// MPRIS MediaPlayer2 interface handlers

DBusHandlerResult MethodHandler::handleRaise_unlocked(DBusConnection* connection, DBusMessage* message) {
    // Bring the media player to the foreground
    // For PsyMP3, this might mean focusing the window or just logging
    std::cout << "MPRIS: Received Raise command - bringing player to foreground" << std::endl;
    
    sendMethodReturn_unlocked(connection, message);
    return DBUS_HANDLER_RESULT_HANDLED;
}

DBusHandlerResult MethodHandler::handleQuit_unlocked(DBusConnection* connection, DBusMessage* message) {
    std::cout << "MPRIS: Received Quit command - shutting down player" << std::endl;
...
DBusHandlerResult MethodHandler::handleSeek_unlocked(DBusConnection* connection, DBusMessage* message) {
    ...
    if (!offset_result.isSuccess()) {
        ...
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.InvalidArgs", 
    ...
    std::cout << "MPRIS: Received Seek command with offset: " << offset_us << " microseconds" << std::endl;
    
    // Calculate new absolute position (current position + offset)
    uint64_t current_position_us = m_properties->getPosition();
    int64_t new_position_us = static_cast<int64_t>(current_position_us) + offset_us;
    ...
    if (track_length_us > 0 && static_cast<uint64_t>(new_position_us) > track_length_us) {
...
DBusHandlerResult MethodHandler::handleSetPosition_unlocked(DBusConnection* connection, DBusMessage* message) {
    auto args_result = parseSetPositionArguments_unlocked(message);
    if (!args_result.isSuccess()) {
        logValidationError_unlocked("SetPosition", "arguments", args_result.getError());
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.InvalidArgs", 
    ...
    if (!track_validation.isSuccess()) {
        logValidationError_unlocked("SetPosition", "track_id", track_validation.getError());
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.InvalidArgs", 
    ...
    if (!position_validation.isSuccess()) {
        logValidationError_unlocked("SetPosition", "position", position_validation.getError());
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.InvalidArgs", 
...
MethodHandler::MethodHandler(Player* player, PropertyManager* properties)
    ...
    (void)player; (void)properties; // Suppress unused parameter warnings
}

MethodHandler::~MethodHandler() {}

DBusHandlerResult MethodHandler::handleMessage(DBusConnection* connection, DBusMessage* message) {
    (void)connection; (void)message; // Suppress unused parameter warnings
    return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
}
...
DBusHandlerResult MethodHandler::handleGetProperty_unlocked(DBusConnection* connection, DBusMessage* message) {
    ...
    std::cout << "MPRIS: Received Get property " << property_name << " on interface " << interface_name << std::endl;
    ...
    try {
        ...
        dbus_message_unref(reply);
        logError_unlocked("handleGetProperty", "Failed to append property: " + std::string(e.what()));
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.Failed", 
...
DBusHandlerResult MethodHandler::handleSetProperty_unlocked(DBusConnection* connection, DBusMessage* message) {
    ...
    if (!dbus_message_iter_next(&args) || dbus_message_iter_get_arg_type(&args) != DBUS_TYPE_VARIANT) {
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.InvalidArgs", 
    ...
    std::cout << "MPRIS: Received Set property " << property_name << " on interface " << interface_name << std::endl;
    ...
    if (prop_name_str == "Volume") {
        ...
        std::cout << "MPRIS: Setting shuffle to " << (shuffle ? "true" : "false") << std::endl;
        // TODO: Implement shuffle control in Player if available
        
    } else {
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.NotSupported", 
...
DBusHandlerResult MethodHandler::handleGetAllProperties_unlocked(DBusConnection* connection, DBusMessage* message) {
    DBusMessageIter args;
    if (!dbus_message_iter_init(message, &args)) {
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.InvalidArgs", 
    ...
    std::cout << "MPRIS: Received GetAll properties for interface " << interface_name << std::endl;
    
    // Create reply message
    DBusMessage* reply = dbus_message_new_method_return(message);
    if (!reply) {
        logError_unlocked("handleGetAllProperties", "Failed to create reply message");
        return DBUS_HANDLER_RESULT_NEED_MEMORY;
    }
    ...
    try {
        ...
        dbus_message_unref(reply);
        logError_unlocked("handleGetAllProperties", "Failed to append properties: " + std::string(e.what()));
        sendErrorReply_unlocked(connection, message, "org.mpris.MediaPlayer2.Error.Failed", 
...
void MethodHandler::sendMethodReturn_unlocked(DBusConnection* connection, DBusMessage* message) {
    DBusMessage* reply = dbus_message_new_method_return(message);
    if (reply) {
        dbus_connection_send(connection, reply, nullptr);
        dbus_message_unref(reply);
    } else {
        logError_unlocked("sendMethodReturn", "Failed to create method return message");
    }
}

void MethodHandler::sendErrorReply_unlocked(DBusConnection* connection, DBusMessage* message, 
...
void MethodHandler::appendPropertyToMessage_unlocked(DBusMessage* reply, const std::string& property_name) {
    DBusMessageIter args, variant_iter;
    dbus_message_iter_init_append(reply, &args);
    ...
    if (property_name == "PlaybackStatus") {
        ...
        for (const auto& [key, value] : metadata_dict) {
            ...
            switch (value.type) {
                case PsyMP3::MPRIS::DBusVariant::String: {
                    ...
                    const char* str_cstr = str_val.c_str();
                    dbus_message_iter_append_basic(&entry_variant_iter, DBUS_TYPE_STRING, &str_cstr);
        ...
        dbus_message_iter_append_basic(&variant_iter, DBUS_TYPE_INT64, &position_val);
        dbus_message_iter_close_container(&args, &variant_iter);
        ...
        dbus_message_iter_open_container(&args, DBUS_TYPE_VARIANT, "s", &variant_iter);
        dbus_message_iter_append_basic(&variant_iter, DBUS_TYPE_STRING, &loop_status);
        dbus_message_iter_close_container(&args, &variant_iter);
...
void MethodHandler::appendAllPropertiesToMessage_unlocked(DBusMessage* reply, const std::string& interface_name) {
    ...
    if (interface_name == MPRIS_PLAYER_INTERFACE) {
        ...
        for (const auto& prop_name : properties) {
            ...
            dbus_message_iter_append_basic(&entry_iter, DBUS_TYPE_STRING, &prop_name_cstr);
            ...
            if (temp_msg) {
                try {
                    ...
                    if (dbus_message_iter_init(temp_msg, &temp_args)) {
                        ...
                        // This is a simplified approach - in practice we'd need to copy the entire variant structure
                        if (prop_name == "PlaybackStatus") {
                            ...
                            dbus_message_iter_append_basic(&variant_iter, DBUS_TYPE_STRING, &placeholder);
                            dbus_message_iter_close_container(&entry_iter, &variant_iter);
                        }
                    }
                } catch (const std::exception& e) {
                    logError_unlocked("appendAllPropertiesToMessage", "Failed to get property " + prop_name + ": " + e.what());
                    // Add empty variant as fallback
                    dbus_message_iter_open_container(&entry_iter, DBUS_TYPE_VARIANT, "s", &variant_iter);
                    const char* empty_str = "";
                    dbus_message_iter_append_basic(&variant_iter, DBUS_TYPE_STRING, &empty_str);
                    dbus_message_iter_close_container(&entry_iter, &variant_iter);
                }
        ...
        dbus_message_iter_append_basic(&entry_iter, DBUS_TYPE_STRING, &identity_key);
        dbus_message_iter_open_container(&entry_iter, DBUS_TYPE_VARIANT, "s", &variant_iter);
        const char* identity_val = "PsyMP3";
        dbus_message_iter_append_basic(&variant_iter, DBUS_TYPE_STRING, &identity_val);
        dbus_message_iter_close_container(&entry_iter, &variant_iter);
        dbus_message_iter_close_container(&dict_iter, &entry_iter);
        ...
        dbus_message_iter_append_basic(&entry_iter, DBUS_TYPE_STRING, &can_quit_key);
        dbus_message_iter_open_container(&entry_iter, DBUS_TYPE_VARIANT, "b", &variant_iter);
        dbus_bool_t can_quit_val = TRUE;
        dbus_message_iter_append_basic(&variant_iter, DBUS_TYPE_BOOLEAN, &can_quit_val);
        dbus_message_iter_close_container(&entry_iter, &variant_iter);
        dbus_message_iter_close_container(&dict_iter, &entry_iter);
        ...
        dbus_message_iter_append_basic(&entry_iter, DBUS_TYPE_STRING, &can_raise_key);
        dbus_message_iter_open_container(&entry_iter, DBUS_TYPE_VARIANT, "b", &variant_iter);
        dbus_bool_t can_raise_val = TRUE;
        dbus_message_iter_append_basic(&variant_iter, DBUS_TYPE_BOOLEAN, &can_raise_val);
        dbus_message_iter_close_container(&entry_iter, &variant_iter);
        dbus_message_iter_close_container(&dict_iter, &entry_iter);
    }
...
void MethodHandler::logMethodCall_unlocked(const std::string& interface_name, const std::string& method_name) {
    std::cout << "MPRIS MethodHandler: " << interface_name << "." << method_name << std::endl;
}

void MethodHandler::logError_unlocked(const std::string& context, const std::string& error_message) {
    std::cerr << "MPRIS MethodHandler Error [" << context << "]: " << error_message << std::endl;
}

void MethodHandler::logValidationError_unlocked(const std::string& method_name, const std::string& parameter, 
                                               const std::string& error_message) {
    std::cerr << "MPRIS MethodHandler Validation Error [" << method_name << "." << parameter << "]: " 
