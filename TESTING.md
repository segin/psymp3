...
- **ffmpeg is an oracle, nothing more.** It encodes the streams and its decode is the reference to diff against; its source is copyleft, under the LGPL or the GPL depending on how it is built, and is never read. PsyMP3's decoder is written from ATSC A/52.
- **One encode, every container.** Each tone or silence stream is encoded once as raw and stream-copied into the containers, so all of them hold bit-identical syncframes. A decode through any demuxer must therefore match the raw decode sample for sample — a mismatch is a demuxer bug. The script re-extracts every container and `cmp`s it against the raw stream, and fails if one differs.
- **What each stream is for:** `ac3_stereo` the common case; `ac3_51` LFE and channel order (each channel carries its own tone — L 220, R 330, C 440, LFE 55, Ls 550, Rs 660 Hz — so a swap is measurable); `ac3_mono` so a desynchronised block cannot hide behind a second channel; `ac3_silence` the degenerate allocation; `ac3_lowrate` frames short enough that the six audio blocks must land within a few bits of the frame end; `eac3_stereo` and `eac3_51` for telling Annex E apart.
- **Broadband streams for `test_ac3_oracle`.** Every tone is at or below 1 kHz, below the first coupled and the first rematrixed bin, so four noise streams cover those tools. `ac3_coupled_noise` and `eac3_coupled_noise` are two independent noise channels coupled from about 4.6 kHz; `ac3_coupled_quiet` is coupled noise whose right channel is the left 30 dB down, at a rate low enough that many coupled bins get no bits and are dithered; `ac3_rematrix_noise` is partly correlated stereo with coupling off, so every rematrixing band carries signal. They stay raw, with a reference decode each, because they test decoding rather than demuxing. `test_ac3_oracle` decodes them and requires every band holding signal to be within 1 dB of ffmpeg's decode. It compares band energies, since both decoders dither at random, and skips when the corpus is absent.
