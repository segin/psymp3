/*
 * mp3out.cpp - MP3 to PCM/WAV decoder utility
 ...
 *
 * This utility decodes MP3 files to either:
 * - Raw PCM (LPCM) data
 * - RIFF WAVE format
 *
...
/**
 * @brief Configuration options for mp3out
 */
struct Mp3OutConfig {
...
static void printUsage(const char* program_name) {
    std::cerr << "Usage: " << program_name << " [options] input.mp3 [output.wav|output.pcm]\n"
              << "\n"
              << "Decode MP3 files to PCM or WAVE format.\n"
              << "\n"
              << "Options:\n"
              << "  -r, --raw       Output raw PCM instead of WAVE (default: WAVE)\n"
              << "  -q, --quiet     Suppress progress output\n"
              ...
              << "\n"
              << "Examples:\n"
              << "  " << program_name << " input.mp3 output.wav\n"
              << "  " << program_name << " -r input.mp3 output.pcm\n"
              << "  " << program_name << " input.mp3 > output.pcm\n"
              << "  " << program_name << " input.mp3 | aplay -f S16_LE -r 44100 -c 2\n";
}

/**
 * @brief Parse command line arguments
 */
static bool parseArgs(int argc, char* argv[], Mp3OutConfig& config) {
 ...
 */
static int decode(const Mp3OutConfig& config) {
    ...
    if (!config.raw_output) {
        if (!writeWAVHeader(*out, sample_rate, channels, output_bits, 0)) {
    ...
    while (!stream->eof()) {
        ...
        if (channels > 0) {
            total_samples += bytes_read / (2 * channels);
        }
