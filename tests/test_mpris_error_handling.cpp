...
private:
    static void testMPRISErrorCreation() {
        ...
        MPRISTypes::MPRISError error1(MPRISTypes::MPRISError::Category::Connection, "Test error");
        assert(error1.getCategory() == MPRISTypes::MPRISError::Category::Connection);
        assert(error1.getMessage() == "Test error");
        assert(error1.getSeverity() == MPRISTypes::MPRISError::Severity::Error);
        assert(error1.getRecoveryStrategy() == MPRISTypes::MPRISError::RecoveryStrategy::None);
        
        // Test full error creation
        MPRISTypes::MPRISError error2(
            MPRISTypes::MPRISError::Category::Threading,
            MPRISTypes::MPRISError::Severity::Critical,
            "Threading error",
            "test_context",
        ...
        assert(error2.getCategory() == MPRISTypes::MPRISError::Category::Threading);
        assert(error2.getSeverity() == MPRISTypes::MPRISError::Severity::Critical);
        assert(error2.getMessage() == "Threading error");
        assert(error2.getContext() == "test_context");
        assert(error2.getRecoveryStrategy() == MPRISTypes::MPRISError::RecoveryStrategy::Restart);
        assert(error2.getDetails() == "Additional details");
        
        // Test error ID uniqueness
        MPRISTypes::MPRISError error3(MPRISTypes::MPRISError::Category::Message, "Another error");
    ...
    static void testMPRISErrorHierarchy() {
        ...
        MPRISTypes::ConnectionError conn_error("Connection failed");
        assert(conn_error.getCategory() == MPRISTypes::MPRISError::Category::Connection);
        assert(conn_error.getRecoveryStrategy() == MPRISTypes::MPRISError::RecoveryStrategy::Reconnect);
        
        MPRISTypes::MessageError msg_error("Invalid message");
        assert(msg_error.getCategory() == MPRISTypes::MPRISError::Category::Message);
        
        MPRISTypes::ThreadingError thread_error("Deadlock detected");
        assert(thread_error.getCategory() == MPRISTypes::MPRISError::Category::Threading);
        assert(thread_error.getSeverity() == MPRISTypes::MPRISError::Severity::Critical);
        
        // Test exception interface
        try {
            throw conn_error;
        } catch (const MPRISTypes::MPRISError& e) {
            assert(e.getCategory() == MPRISTypes::MPRISError::Category::Connection);
    ...
    static void testErrorLogger() {
        ...
        auto& logger2 = MPRISTypes::ErrorLogger::getInstance();
        ...
        logger.setLogLevel(MPRISTypes::ErrorLogger::LogLevel::Error);
        assert(logger.getLogLevel() == MPRISTypes::ErrorLogger::LogLevel::Error);
        
        // Test error logging
        MPRISTypes::MPRISError test_error(MPRISTypes::MPRISError::Category::Connection, "Test connection error");
        ...
        logger.setLogHandler([&](MPRISTypes::ErrorLogger::LogLevel level,
                                MPRISTypes::MPRISError::Category category,
    ...
    static void testErrorRecoveryManager() {
        ...
        MPRISTypes::ErrorRecoveryManager::RecoveryConfig config;
        config.max_attempts = 5;
        config.initial_delay = std::chrono::milliseconds(100);
        config.max_delay = std::chrono::milliseconds(1000);
        config.backoff_multiplier = 2.0;
        
        recovery_manager.setRecoveryConfig(MPRISTypes::MPRISError::Category::Connection, config);
        
        auto retrieved_config = recovery_manager.getRecoveryConfig(MPRISTypes::MPRISError::Category::Connection);
    ...
    static void testGracefulDegradationManager() {
        ...
        degradation_manager.setDegradationLevel(MPRISTypes::GracefulDegradationManager::DegradationLevel::None);
        ...
        degradation_manager.setDegradationLevel(MPRISTypes::GracefulDegradationManager::DegradationLevel::Minimal);
        assert(degradation_manager.getDegradationLevel() == MPRISTypes::GracefulDegradationManager::DegradationLevel::Minimal);
    ...
    static void testErrorRecoveryScenarios() {
        ...
        MPRISTypes::ConnectionError conn_error("Connection lost");
        ...
        MPRISTypes::ErrorRecoveryManager::RecoveryConfig threading_config;
        threading_config.max_attempts = 1; // Very limited for threading errors
        recovery_manager.setRecoveryConfig(MPRISTypes::MPRISError::Category::Threading, threading_config);
    ...
    static void testDegradationScenarios() {
        ...
        for (int i = 0; i < 3; ++i) {
            MPRISTypes::ConnectionError error("Connection error " + std::to_string(i));
            degradation_manager.reportError(error);
        }
        
        // Scenario 2: Critical error causing immediate full degradation
        degradation_manager.setDegradationLevel(MPRISTypes::GracefulDegradationManager::DegradationLevel::None);
        degradation_manager.setErrorThreshold(MPRISTypes::MPRISError::Category::Threading, 1);
        
        MPRISTypes::ThreadingError critical_error("Critical threading error");
    ...
    static void testErrorStatistics() {
        ...
        std::vector<MPRISTypes::MPRISError> test_errors = {
            MPRISTypes::ConnectionError("Connection error 1"),
            MPRISTypes::ConnectionError("Connection error 2"),
            MPRISTypes::MessageError("Message error 1"),
            MPRISTypes::PlayerStateError("Player state error 1"),
            MPRISTypes::ThreadingError("Threading error 1")
        };
