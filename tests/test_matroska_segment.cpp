...
/// @name A small EBML writer
/// Building the bytes rather than shipping a .mkv keeps every value in these
/// tests visible at the call site, and means a case can be written for a file
/// no muxer would produce.
/// @{

std::vector<uint8_t> idBytes(uint32_t id)
{
...
std::vector<uint8_t> sizeBytes(uint64_t size)
{
...
std::vector<uint8_t> element(uint32_t id, const std::vector<uint8_t>& payload)
{
...
std::vector<uint8_t> uintPayload(uint64_t value)
{
    std::vector<uint8_t> out;
    bool started = false;
...
std::vector<uint8_t> uintEl(uint32_t id, uint64_t value)
{
    return element(id, uintPayload(value));
}

std::vector<uint8_t> strEl(uint32_t id, const std::string& value)
{
    return element(id, std::vector<uint8_t>(value.begin(), value.end()));
}

std::vector<uint8_t> floatEl(uint32_t id, double value)
{
    uint64_t bits = 0;
...
std::vector<uint8_t> ebmlHeader(const std::string& doc_type)
{
